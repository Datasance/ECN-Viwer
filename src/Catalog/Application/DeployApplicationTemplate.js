    ...
    <div className={classes.container}>
      <div className={classes.wrapper}>
        ...
        {Object.keys(variables).map((key) => {
          ...
          return v.readOnly ? (
            <div
              className={classes.variableSection}
              style={{ width: "400px" }}
              key={key}
            >
              <div>
                Variable <strong>{key}</strong> is not configurable using ECN
                Viewer
              </div>
              <div>
                Default value is{" "}
                <strong>{JSON.stringify(v.defaultValue)}</strong>
              </div>
            </div>
          ) : v.isAgentName ? (
        ...
        <div className={classes.buttonContainer}>
          <Button style={{ marginRight: "15px" }} onClick={close}>
            Cancel
          </Button>
          {loading ? (
            ...
            <Button
              variant="contained"
              color="primary"
              onClick={() => deployApplication(variables, template)}
            >
              Deploy
            </Button>
          )}
