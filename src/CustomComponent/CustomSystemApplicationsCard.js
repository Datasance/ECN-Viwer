...
const CustomSystemApplicationsCard = ({ activeAgents = [], systemApplications = [], cardTitle = 'System Applications Overview' }) => {
    const [structuredApplications, setStructuredApplications] = useState([]);

    useEffect(() => {

        const structured = systemApplications.map(app => ({
            agentName: activeAgents.length > 0 
                ? activeAgents.find(agent => agent.uuid === app.microservices[0].iofogUuid)?.name || "Unknown Agent" 
                : "Unknown Agent",
            appName: app.name,
    ...
    }, [activeAgents, systemApplications]);
    ...
    if (!systemApplications.length) {
        return (
            <div className="bg-gray-800 rounded-2xl p-6 shadow-md w-full h-full flex flex-col justify-center items-center">
                <div className="text-gray-400 text-lg">No System Applications Available</div>
            </div>
        );
    }
                ...
                <div className="flex flex-col space-y-4">
                    {structuredApplications.map((app, idx) => (
                        <div key={idx} className="bg-gray-700 rounded-lg p-4 shadow">
                            <div className="text-white text-md font-bold mb-2">{app.agentName}</div>
                            <div className="text-white text-sm font-semibold mb-2">{app.appName}</div>
...
export default CustomSystemApplicationsCard;
