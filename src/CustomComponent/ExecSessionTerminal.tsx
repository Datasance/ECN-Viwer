...
const errorMessages: Record<string, string> = {
    "No available exec session": "No available exec session for this agent or microservice. Be sure to attach/link exec session to the agent or microservice first. If you already attached/linked exec session to the agent or microservice, please wait for the exec session to be ready.",
    "Microservice has already active exec session": "Another user is already connected to this microservice. Only one user can connect at a time.",
    "Timeout waiting for agent connection": "Timeout waiting for agent connection. Please ensure the microservice/agent is running and try again.",
    "Authentication failed": "Authentication failed. Please check your credentials and try again.",
    "Microservice is not running": "Microservice is not running. Please start the microservice first.",
    "Microservice exec is not enabled": "Microservice exec is not enabled. Please enable exec for this microservice.",
    "Microservice already has an active session": "Another user is already connected to this microservice. Only one user can connect at a time.",
    "Insufficient permissions": "Insufficient permissions. Required roles: SRE for Node Exec or Developer for Microservice Exec.",
    "Only SRE can access system microservices": "Only SRE can access system microservices. Please contact your administrator.",
};
...
const ExecSessionTerminal: React.FC<ExecSessionTerminalProps> = ({
    ...
    useEffect(() => {
        const term = new Terminal({
            cursorBlink: true,
            fontSize: 14,
            fontFamily: '"Fira Code", "JetBrains Mono", "Cascadia Code", "SF Mono", "Monaco", "Inconsolata", "Roboto Mono", "Source Code Pro", monospace',
            fontWeight: '400',
            lineHeight: 1.2,
            letterSpacing: 0.5,
            convertEol: true,
