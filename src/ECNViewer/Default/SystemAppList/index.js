...
export default function SystemAppList ({ systemApplications: unfilteredApplications, filter, loading, application, agents, selectApplication, setAgent }) {
  ...
  const [applicationMenu, setApplicationMenu] = React.useState(unfilteredApplications !== undefined ? unfilteredApplications[0] : {})
      ...
      <Table stickyHeader>
        <TableHead>
          <TableRow>
            ...
            <TableCell className={classes.tableTitle} classes={{ stickyHeader: classes.stickyHeader, root: classes.headerCell }}>Agent</TableCell>
          ...
          {(loading ? (new Array(5)).fill(0).map((v, idx) => <TableRow key={idx}><TableCell colSpan={4}><Skeleton height={29} /></TableCell></TableRow>) : applications?.map(a => {
            return (
              <TableRow button key={a.uuid} classes={{ hover: classes.tableRowHover }} hover>
                <TableCell onClick={() => selectApplication({...a, "FromSystemApplication":true})} classes={{ root: classes.tableCell }}>
                ...
                <TableCell
                      onClick={() => setAgent(agents[0])}
                      classes={{ root: classes.tableCell }}
                    >
                      <div style={{ display: "flex", alignItems: "center" }}>
                        <Status status={agents[0]?.daemonStatus} />
                        <span
